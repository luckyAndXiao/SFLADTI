...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, **config):
        ...
        # self.var_loss = {"epoch": [], "train loss": [], "val loss": []}
        self.df_tps = None
    ...
    def save_result(self):
        ...
        loss_state = {
            "epoch": [i for i in range(1, self.epochs + 1)],
            "train loss": self.train_loss_epoch,
            "val loss": self.val_loss_epoch,
            "val auroc": self.val_auroc_epoch
        }


        df_tps_file = os.path.join(self.output_dir, "true_pred_score.csv")
        fusion_file = os.path.join(self.output_dir, "fusion_featrue.csv")
        loss_state_file = os.path.join(self.output_dir, "loss_state.csv")
        ...
        pd.DataFrame(loss_state).to_csv(loss_state_file, index=False)
    ...
    def train_epoch(self):
        ...
        # for i, (v_s, v_d, v_p, labels) in enumerate(self.train_dataloader):
            self.step += 1
